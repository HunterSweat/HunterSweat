 ...
 * Control: Text.
 */
wp.customize.controlConstructor['arilewp-text'] = wp.customize.Control.extend( {
 ...
 */
;
/* global dimensionsarilewpL10n */
/* global arilewpControlLoader */
wp.customize.controlConstructor['arilewp-dimensions'] = wp.customize.arilewpDynamicControl.extend( {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.arilewpValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionsarilewpL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionsarilewpL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global arilewpControlLoader */
wp.customize.controlConstructor['arilewp-background'] = wp.customize.Control.extend( {
	...
	initArileWPControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.arilewp-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['arilewp-editor'] = wp.customize.arilewpDynamicControl.extend( {
	...
	initArileWPControl: function () {
		...
		var id = 'arilewp-editor-' + control_id;
